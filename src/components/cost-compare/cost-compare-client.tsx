...
import { Download, Info, Lock, RefreshCcw, Wallet } from "lucide-react";
...
export function CostCompareClient() {
  ...
  // SalesCompareBar keys on `sales`; map cost -> sales so it renders cost.
  const bySite = useMemo(
    () =>
      [...filteredSites]
        .sort((a, b) => b.cost - a.cost)
        .map((s) => ({ label: `${s.projectCode} · ${s.locationName}`, sales: s.cost, key: s.locationId })),
    [filteredSites],
  );
  const byProject = useMemo(
    () => buildSalesByProject(filteredSites).map((p) => ({ label: p.projectCode, sales: p.cost, key: p.projectCode })),
    [filteredSites],
  );
  const trend = useMemo(
  ...
  const totalCost = filteredSites.reduce((s, x) => s + x.cost, 0);
  const totalSales = filteredSites.reduce((s, x) => s + x.sales, 0);
  const costRatio = totalSales === 0 ? 0 : (totalCost / totalSales) * 100;
  const avgPerSite = filteredSites.length ? totalCost / filteredSites.length : 0;
  const periodDays = daysBetween(filters.from, filters.to);
  const canExport = persona.capabilities.canExport;

  const barData = dimension === "site" ? bySite : byProject;

  return (
    <div>
      <PageHeader
        title="Grafik Perbandingan Cost"
        description="Bandingkan pengeluaran antar site, project, dan waktu."
        breadcrumbs={[{ label: "Overview" }, { label: "Perbandingan Cost" }]}
        actions={
          <>
            <ActivePeriodBadge />
      ...
      <div className="space-y-6 p-4 md:p-6">
        ...
        <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <KpiCard label="Cost Ratio" value={costRatio} format="percent" icon={Wallet} tone={costRatio > 55 ? "danger" : "success"} sub="cost / sales" />
          <KpiCard label="Jumlah Site" value={filteredSites.length} format="number" icon={Wallet} tone="muted" />
        </section>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trend} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="cost" name="Cost" stroke="hsl(38 92% 50%)" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
